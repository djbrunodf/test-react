...
describe('<Home />', () => {
  ...
  it('should search for posts', async () => {
    ...
    expect(screen.queryByRole('heading', { name: 'title3 3' })).not.toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'searchValue: title1' })).toBeInTheDocument();

    userEvent.clear(search);
    expect(screen.getByRole('heading', { name: 'title1 1' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'title2 2' })).toBeInTheDocument();

    userEvent.type(search, 'asjhbdahjdgbqhi');
    expect(screen.getByText('Não existe posts')).toBeInTheDocument();
  });
  it('should load more posts when button click', async () => {
    const { debug } = render(<Home />);
    const noMorePosts = screen.getByText('Não existe posts');
